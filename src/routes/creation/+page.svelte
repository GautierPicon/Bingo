    ...
    <main class="flex flex-col items-center justify-center p-6 pb-20">
        <div bind:this={formRef} class="w-full max-w-lg">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
                {#if errorMessage}
                    <div class="mb-6 rounded-xl bg-red-50 p-4 text-sm font-medium text-red-600 border border-red-100">
                        {errorMessage}
                    </div>
                {/if}
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        <label for="groupName" class="text-xs font-bold uppercase tracking-wider text-slate-400">
                            Nom du salon
                        </label>
                        <input
                            id="groupName"
                            ...
                            disabled={isCreating}
                            class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 text-lg font-semibold transition-all focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none disabled:opacity-50"
                        />
                    </div>

                    <div class="space-y-2">
                        <label for="playerName" class="text-xs font-bold uppercase tracking-wider text-slate-400">
                            Votre pseudonyme
                        </label>
                        <input
                            id="playerName"
                    ...
                    <button
                        ...
                        class="flex w-full items-center justify-between rounded-2xl border border-slate-100 bg-slate-50/50 p-4 transition-all hover:bg-slate-50"
                    >
                        ...
                        <div 
                            class="relative h-6 w-11 rounded-full transition-colors {$useStar ? 'bg-indigo-600' : 'bg-slate-300'}"
                        >
                            <div class="absolute top-1 left-1 h-4 w-4 rounded-full bg-white transition-transform {$useStar ? 'translate-x-5' : 'translate-x-0'}"></div>
                        </div>
                    </button>
